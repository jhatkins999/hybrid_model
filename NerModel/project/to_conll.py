...
class ConllParser():
    ...
    def __init__(self, data_folder_path, scispacy_parser):
        ...
        self.datasets_path = os.path.join(self.data_folder_path, "data_sets.json")
    ...
    def build_publication_to_datasets_and_mentions(self):
        ...
        with open(self.citations_path) as json_citations_file:
            citation_pairs = json.load(json_citations_file, strict=False)
            for pair in citation_pairs:
                pub_id = pair["publication_id"]
                if pub_id in publication_to_datasets_and_mentions:
                    publication_to_datasets_and_mentions[pub_id].append((pair["data_set_id"].split("_")[-1],
                                                                         pair["mention_list"]))
                else:
    ...
    def parse_publication(self, publication):
        ...
        try:
            ...
            publication_text_path = os.path.join(self.text_files_path, str(publication_id) + ".pdf.txt")
