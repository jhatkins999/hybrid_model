...
def get_scispacy_doc(data_folder_path, publication_id, scispacy_parser):
    ...
    if os.path.exists(spacy_doc_dir):
        try:
            ...
            pass
    if not doc:
        with open(os.path.join(data_folder_path,"input","files","text", str(publication_id) + ".pdf.txt"), mode='rt') as txt_fp:
...
def featurize_candidate_datasets(is_test,
    ...
    for candidate_dataset_id in row["candidate_dataset_ids"]:
        ...
        label = candidate_dataset_id == gold_dataset_id
        if candidate_dataset_id in prior_entity_probs:
            prior_entity_probability = prior_entity_probs[candidate_dataset_id]
        else:
            prior_entity_probability = 0

        if candidate_dataset_id in entity_to_prob:
            prior_entity_given_mention_prob = entity_to_prob[candidate_dataset_id]
        else:
            prior_entity_given_mention_prob = 0

        if candidate_dataset_id in prior_mention_given_entity_probs:
            if mention_text in prior_mention_given_entity_probs[candidate_dataset_id]:
                prior_mention_given_entity_prob = prior_mention_given_entity_probs[candidate_dataset_id][mention_text]
            else:
                prior_mention_given_entity_prob = 0
        else:
            prior_mention_given_entity_prob = 0
        year_match = year_match_nearby(mention_contexts, dataset_id_to_kb_entry[candidate_dataset_id])
        ...
        context_word_overlap = context_word_overlap_count(mention_contexts, dataset_id_to_kb_entry[candidate_dataset_id])
        # avoid log(0)
...
def create_dataset_input(rule_based_candidates, mention_context_cache_path, data_folder_path, overall_output_path=None, is_test=False, output_path=None, overwrite_dataset=False):
    ...
    glove_path = os.path.abspath(os.path.join("project", "data", "glove", "glove.6B.50d.txt"))
